function [isUpdated, newCenters, totalDistortion] = estimateNewKCenters(U, Dist, NetMatrix, centers)
%No of clusters
M = length(centers);
...
for i=1:M
    ...
    fprintf('[KMean] Current distortion of cluster[%d]=%g \n',i, minTotalDistortion);
    for j=1:N
        if C{i}(j)~=d(i)
            %find distnaces to all other vertices
            [~, totalCost] = dijkstraBulk(NetMatrix, C{i}(j), C{i});
            ...
            if newTotalDistortion < minTotalDistortion
                ...
                newCenters(i) = C{i}(j);
                isUpdated=1;
                fprintf('[KMean] Found a min distortion of node[%d]=%g !!!\n',C{i}(j), newTotalDistortion);
%             else
