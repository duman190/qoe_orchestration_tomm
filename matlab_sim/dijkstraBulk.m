function [shortestPath, totalCost] = dijkstraBulk(netCostMatrix, s, d)
%==============================================================
...
shortestPath = cell(length(d), 1);
totalCost = [];%cell(length(d), 1);
for i=1:length(d)
    shortestPath{i} = [];
    if parent(d(i)) ~= 0   % if there is a shortestPath!
        t = d(i);
        shortestPath{i} = [d(i)];
        while t ~= s
            p = parent(t);
            shortestPath{i} = [p shortestPath{i}];
    ...
    totalCost = [totalCost distance(d(i))];
end;
%return;