...
void Topology::BriteCSVOutput(char* filename) {
//write nodes info first
  ofstream outfile_nodes;
  string nodes_name = filename;
  nodes_name += "_nodes.csv";
  outfile_nodes.open(nodes_name.c_str(), ios::out);
  outfile_nodes.setf(ios::fixed, ios::floatfield);
  outfile_nodes.precision(2);
  assert(outfile_nodes);

  for (int i = 0; i < g->GetNumNodes(); i++) {
    outfile_nodes << g->GetNodePtr(i)->GetId() << ","
	    << g->GetNodePtr(i)->GetNodeInfo()->GetCoordX() << ","
	    << g->GetNodePtr(i)->GetNodeInfo()->GetCoordY() << ","
	    << g->GetNodePtr(i)->GetInDegree() << ","
	    << g->GetNodePtr(i)->GetOutDegree() << "\n";
...
//write edges info 
  ofstream outfile_edges;
  string edges_name = filename;
  edges_name += "_edges.csv";
  outfile_edges.open(edges_name.c_str(), ios::out);
  outfile_edges.setf(ios::fixed, ios::floatfield);
  outfile_edges.precision(2);
  assert(outfile_edges);

  list<Edge*>::iterator el;
  for (el = g->edges.begin(); el != g->edges.end(); el++) {

    outfile_edges << (*el)->GetId() << ","
	    << (*el)->GetSrc()->GetId() << ","
	    << (*el)->GetDst()->GetId() << ","
	    << (*el)->Length() << ",";
    ...
    switch((*el)->GetConf()->GetEdgeType()) {
    case EdgeConf::RT_EDGE:
      outfile_edges  << ((RouterEdgeConf*)((*el)->GetConf()))->GetDelay() << ","
	       << (*el)->GetConf()->GetBW() << "\n";
      break;

    case EdgeConf::AS_EDGE:
      outfile_edges  << -1   /* No delay for AS Edges */ << ","
	       << (*el)->GetConf()->GetBW() << "\n";
      break;
