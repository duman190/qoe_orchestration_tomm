...
edges_array = csvread(strcat(topo,edges_file));
...
for i=1:size(edges_array,1)
    newEdge = edge(edges_array(i,1)+1, edges_array(i,2)+1, edges_array(i,3)+1, edges_array(i,4), edges_array(i,6), 1);
    edges(i,1) = newEdge;
...
U = PCM_U;
dlmwrite(strcat('pcm_',num2str(M),'_servers.txt'),U,'delimiter','\t','precision',4);
C=[];
for j=1:M
    C=[C PCM_centers(j).id];
end;
dlmwrite(strcat('pcm_',num2str(M),'_servers.txt'),C,'delimiter','\t','-append');
end